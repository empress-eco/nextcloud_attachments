	...
	add_migrate_button: function (frm) {
		if (frm.doc.enabled && frm.doc.nextcloud_username && frm.doc.password) {
			frm.add_custom_button(__("Migrate Now"), function() {
				frappe.call({
					method: "attachments_on_nextcloud.migrate_to_nextcloud",
					freeze: true
				});
	...
	add_backup_button: function(frm) {
		if (frm.doc.enabled && frm.doc.nextcloud_username && frm.doc.password) {
			frm.add_custom_button(__("Backup Now"), function() {
				frappe.call({
					method: "attachments_on_nextcloud.nextcloud_integration.doctype.nextcloud_setting.nextcloud_setting.take_backup",
					freeze: true
				});
			}).addClass("btn-primary");
		}
